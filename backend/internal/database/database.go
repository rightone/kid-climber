...
func InitDatabase(databasePath string) error {
	...
	// 初始化预设组件数据
	if err := initPresetComponents(); err != nil {
		return fmt.Errorf("initialize preset components: %w", err)
	}
...
func initPresetComponents() error {
	// 检查是否已有组件数据
	var count int64
	if result := DB.Model(&models.Component{}).Count(&count); result.Error != nil {
		return result.Error
	}
	if count > 0 {
		return nil

...
import type { ComponentColor, Design, ComponentInstance } from '../types';
import { normalizeComponentInstanceColor } from '../systems/PipeColorSystem';
...
interface SerializedConnection {
  id: string;
  source: Design['connections'][number]['source'];
  target: Design['connections'][number]['target'];
  type: string;
}

interface SerializedDesignFile {
  version: string;
  metadata: {
    name: string;
    description?: string;
  ...
  settings?: Partial<Design['settings']>;
}
  ...
  return new Promise((resolve) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const designData = JSON.parse(content) as SerializedDesignFile;
        ...
        const design: Design = {
          ...
          productProfileVersion: designData.metadata.productProfileVersion,
          status: 'draft',
          ...
          settings: {
            gridSize: designData.settings?.gridSize ?? 20,
            showConnections: designData.settings?.showConnections ?? true,
            viewMode: designData.settings?.viewMode ?? 'realistic',
          },
...
// 验证设计文件
export const validateDesignFile = (data: unknown): boolean => {
  if (!data || typeof data !== 'object') {
    return false;
  }

  const candidate = data as Partial<SerializedDesignFile>;
  if (!candidate.version || !candidate.metadata) {
    return false;
  }

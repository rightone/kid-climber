...
import {
  componentIdFromMaterialKey,
  getMaterialVariantDescriptor,
  materialVariantLabelsFromKey,
} from '../systems/MaterialVariantSystem';
...
export const calculateMaterialRequirement = (
  ...
  components.forEach(component => {
    const materialKey = getMaterialVariantDescriptor(component).materialKey;
    if (!requirement[materialKey]) {
      requirement[materialKey] = {
        required: 0,
        available: inventory[materialKey] || 0,
        shortage: 0,
      };
    }
    requirement[materialKey].required++;
  });
...
export const calculateTotalCost = (
  ...
  Object.entries(materials).forEach(([componentId, data]) => {
    const price = prices[componentId] || prices[componentIdFromMaterialKey(componentId)] || 0;
...
export const generateMaterialList = (
  ...
  Object.entries(materials).forEach(([componentId, data]) => {
    const baseComponentId = componentIdFromMaterialKey(componentId);
    const componentDef = getComponentById(baseComponentId);
    const price = prices[componentId] || prices[baseComponentId] || 0;
    const variantLabels = materialVariantLabelsFromKey(componentId);
    
    list.push({
      componentId,
      componentName: `${componentDef?.name || baseComponentId}${variantLabels.length > 0 ? `·${variantLabels.join('·')}` : ''}`,
      ...
      specifications: [getComponentSpecifications(baseComponentId), ...variantLabels]
        .filter(Boolean)
        .join(', '),
    });
  });
...
export const calculateStability = (components: ComponentInstance[]): StabilityResult => {
  ...
  // 检查是否有平台
  const platforms = components.filter(c =>
    c.componentId.startsWith('platform') || c.componentId.startsWith('board_')
  );
  ...
  const connectors = components.filter(c => 
    c.componentId.startsWith('connector_') ||

...
import type { ComponentDefinition, BoardStyle, ComponentInstance } from '../../../types';
import {
  getPipeCenterlineSpec,
  getReferenceBoardSize,
...
// 接头几何统一由组件库连接点定义生成，确保真实构件、预览和缩略图方向一致。
export const createConnectorGeometryFromDefinition = (
  ...
  const armRadius = radius * REFERENCE_PRODUCT_SPEC.connectors.armOuterDiameterScale;
  const collarRadius = REFERENCE_PRODUCT_SPEC.connectors.collarDiameterCm / 2;
  const collarLength = REFERENCE_PRODUCT_SPEC.connectors.collarLengthCm;

  componentDef.connectionPoints
    .filter(point => point.role !== 'board-mount')
    .forEach(point => {
    const endpoint = new THREE.Vector3(...point.position);
    const armLength = endpoint.length();
    if (armLength < 0.001) return;

    const armDirection = new THREE.Vector3(...point.direction).normalize();
    ...
    arm.quaternion.setFromUnitVectors(cylinderAxis, armDirection);
    arm.position.copy(endpoint).multiplyScalar(0.5);
    group.add(arm);
    ...
    collar.quaternion.setFromUnitVectors(cylinderAxis, armDirection);
    collar.position.copy(endpoint).addScaledVector(
      armDirection,
      -collarLength / 2
    );
    group.add(collar);
  ...
  group.add(
    new THREE.Mesh(new THREE.SphereGeometry(
      radius * REFERENCE_PRODUCT_SPEC.connectors.hubOuterDiameterScale,
      segments,
      segments
    ))
  );
...
const createExtrudedHorizontalShape = (
  shape: THREE.Shape,
  thickness: number
): THREE.BufferGeometry => {
...
const createBoardBodyGeometry = (
  ...
  if (style === 'perforated') {
    const { radiusCm: holeRadius, pitchCm: holePitch, edgeMarginCm: edgeMargin } =
      REFERENCE_PRODUCT_SPEC.boards.perforation;
    for (
      let x = -halfWidth + edgeMargin;
      x <= halfWidth - edgeMargin + 1e-6;
      x += holePitch
    ) {
      for (
        let z = -halfHeight + edgeMargin;
        z <= halfHeight - edgeMargin + 1e-6;
        z += holePitch
      ) {
  ...
  return createExtrudedHorizontalShape(outer, thickness);
};
...
export const createBoardGeometry = (
  ...
  const style = options?.style === 'perforated' ? 'perforated' : 'solid';
  const boardSpec = REFERENCE_PRODUCT_SPEC.boards;
  const thickness = boardSpec.thicknessCm;
  const insetWidth = Math.max(
    boardSpec.minimumInsetCm,
    nominalWidth - boardSpec.insetCm
  );
  const insetHeight = Math.max(
    boardSpec.minimumInsetCm,
    nominalHeight - boardSpec.insetCm
  );
  const pipeRadius = REFERENCE_PRODUCT_SPEC.pipes.outerDiameterCm / 2;
  const topRecess = boardSpec.pipeCrownRecessCm;
  const centerY = pipeRadius - topRecess - thickness / 2;
  const group = new THREE.Group();

  const bodyGeometry = createBoardBodyGeometry(
    insetWidth,
    insetHeight,
    thickness,
    style
  );
  bodyGeometry.translate(0, centerY, 0);
  group.add(new THREE.Mesh(bodyGeometry));

  const nodeHalfWidth = nominalWidth / 2;
  const nodeHalfHeight = nominalHeight / 2;
  const bodyHalfWidth = insetWidth / 2;
  const bodyHalfHeight = insetHeight / 2;
  const earPositions: Array<[number, number]> = [
    [-nodeHalfWidth, -nodeHalfHeight],
    [nodeHalfWidth, -nodeHalfHeight],
    [nodeHalfWidth, nodeHalfHeight],
    [-nodeHalfWidth, nodeHalfHeight],
  ];
  earPositions.forEach(([x, z]) => {
    const pad = new THREE.Shape();
    pad.absarc(x, z, boardSpec.earOuterRadiusCm, 0, Math.PI * 2, false);
    const relief = new THREE.Path();
    relief.absarc(x, z, boardSpec.earReliefRadiusCm, 0, Math.PI * 2, false);
    pad.holes.push(relief);
    const padGeometry = createExtrudedHorizontalShape(pad, thickness);
    padGeometry.translate(0, centerY, 0);
    group.add(new THREE.Mesh(padGeometry));

    const bodyCornerX = Math.sign(x) * bodyHalfWidth;
    const bodyCornerZ = Math.sign(z) * bodyHalfHeight;
    const bridgeWidth = Math.abs(x - bodyCornerX) + boardSpec.bridgeOverlapCm;
    const bridgeDepth = Math.abs(z - bodyCornerZ) + boardSpec.bridgeOverlapCm;
    const bridge = new THREE.Mesh(
      new THREE.BoxGeometry(bridgeWidth, thickness, bridgeDepth)
    );
    bridge.position.set(
      (x + bodyCornerX) / 2,
      centerY,
...
export const createUCurveGeometry = (
  bendRadius: number,
  tubeRadius: number
): THREE.BufferGeometry => {
  class UCurveCenterline extends THREE.Curve<THREE.Vector3> {
    constructor() {
      super();
    }
    ...
    override getPoint(t: number, target = new THREE.Vector3()): THREE.Vector3 {
      ...
      return target.set(
        bendRadius * Math.cos(theta),
        0,
        bendRadius * Math.sin(theta)
      );
    }
  }

  return new THREE.TubeGeometry(
    new UCurveCenterline(),
    64,
    tubeRadius,
    16,
    false
...
// A reference ramp is authored flat around its component origin. The
// multi-anchor placement transform supplies the fixed incline, so the visual
// mesh and the two upper mounting ports share one rigid physical model.
export const createRampGeometry = (
  width: number,
  length: number,
  _rise: number,
  thickness: number = REFERENCE_PRODUCT_SPEC.boards.thicknessCm
): THREE.BufferGeometry => {
  const safeWidth = Math.max(width, 8);
  const safeLength = Math.max(length, 8);
  const bodyWidth = Math.max(
    8,
    safeWidth - REFERENCE_PRODUCT_SPEC.boards.insetCm
  );
  const group = new THREE.Group();

  const deck = new THREE.Mesh(
    new THREE.BoxGeometry(bodyWidth, thickness, safeLength)
  );
  group.add(deck);

  // Low raised edges express the real slide/ramp profile without inventing a
  // large placeholder tube. They are part of the same rigid inclined body.
  const railWidth = Math.min(2.2, bodyWidth * 0.08);
  const railHeight = Math.max(2.8, thickness * 1.5);
  [-1, 1].forEach(side => {
    const rail = new THREE.Mesh(
      new THREE.BoxGeometry(railWidth, railHeight, safeLength)
    );
    rail.position.set(
      side * (bodyWidth / 2 - railWidth / 2),
      thickness / 2 + railHeight / 2,
      0
    );
    group.add(rail);
  });

  // Two compact upper mounting ears reach the 40cm node pitch while the ramp
  // body remains inset between the frame tubes.
  const earReach = Math.max(0, (safeWidth - bodyWidth) / 2);
  if (earReach > 0.001) {
    const earDepth = Math.min(7, safeLength * 0.16);
    [-1, 1].forEach(side => {
      const ear = new THREE.Mesh(
        new THREE.BoxGeometry(
          earReach + REFERENCE_PRODUCT_SPEC.boards.bridgeOverlapCm,
          thickness,
          earDepth
        )
      );
      ear.position.set(
        side * (bodyWidth / 2 + earReach / 2),
        0,
        -safeLength / 2 + earDepth / 2
      );
      group.add(ear);
    });
  }
...
const mergeGeometries = (geometries: THREE.BufferGeometry[]): THREE.BufferGeometry => {
  ...
  geometries.forEach((geometry) => {
    // A merged geometry must use one indexing strategy. Board bodies created by
    // ExtrudeGeometry are non-indexed while their mounting bridges are indexed;
    // mixing both and retaining only the available indices makes the body vanish.
    const source = geometry.index ? geometry.toNonIndexed() : geometry;
    const posArray = source.attributes.position.array;
    const normalArray = source.attributes.normal.array;
...
export const createComponentGeometry = (
  componentId: string,
  componentDef: ComponentDefinition,
  instance?: Pick<ComponentInstance, 'properties' | 'color'>
): THREE.BufferGeometry => {
  // 解析组件类型
  const parts = componentId.split('_');
  const type = parts[0];
  const diameter = componentDef.diameter || 2.5;
  const geometryKind = (componentDef.properties as { geometryKind?: string } | undefined)?.geometryKind;

  if (geometryKind === 'u-curve-180') {
    const centerline = getPipeCenterlineSpec(componentId);
    const bendRadius = centerline?.kind === 'circular-arc'
      ? centerline.radius ?? REFERENCE_PRODUCT_SPEC.pipes.uCurve40cm.centerlineRadiusCm
      : REFERENCE_PRODUCT_SPEC.pipes.uCurve40cm.centerlineRadiusCm;
    return createUCurveGeometry(bendRadius, diameter / 2);
  }

  if (geometryKind === 'ramp-panel' || geometryKind === 'reference-ramp') {
    const properties = componentDef.properties as {
      rampLengthCm?: number;
      rampRiseCm?: number;
      rampWidthCm?: number;
      rampThicknessCm?: number;
    } | undefined;
    return createRampGeometry(
      properties?.rampWidthCm ?? componentDef.width ?? 40,
      properties?.rampLengthCm ?? componentDef.length ?? 45,
      properties?.rampRiseCm ?? componentDef.height ?? 20,
      properties?.rampThicknessCm ?? REFERENCE_PRODUCT_SPEC.boards.thicknessCm
    );
  }
  ...
  if (
    componentDef.category === 'connector' &&
    componentDef.connectionPoints.length > 0
  ) {
    return createConnectorGeometryFromDefinition(componentDef, diameter);
  }
  ...
  if (type === 'platform' || type === 'board') {
    const boardStyle =
      (instance?.properties as { boardStyle?: BoardStyle })?.boardStyle ??
      (componentDef.properties as { boardStyle?: BoardStyle })?.boardStyle ??
      'solid';
    const referenceBoardSize = getReferenceBoardSize(componentDef.id);
    return createBoardGeometry(
      referenceBoardSize?.widthCm ?? componentDef.width ?? 40,
      referenceBoardSize?.heightCm ?? componentDef.height ?? 40,
      { style: boardStyle }
    );
  }

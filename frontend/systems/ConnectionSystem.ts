...
export class ConnectionSystem {
  ...
  private maxSnapDistance: number = 15; // 最大吸附距离（厘米）
  ...
  // 自动连接组件
  autoConnect(components: ComponentInstance[]): { id: string; source: any; target: any; type: string; isActive: boolean }[] {
    const connections: { id: string; source: any; target: any; type: string; isActive: boolean }[] = [];
    
    // 更新连接点
    this.updateConnectionPoints(components);
    
    // 遍历所有组件
    components.forEach((component) => {
      const componentPoints = this.connectionPoints.get(component.instanceId) || [];
      
      componentPoints.forEach((point) => {
        // 查找附近的连接点
        const nearbyPoints = this.findNearbyConnectionPoints(
          point.position,
          point.instanceId
        );
        ...
        nearbyPoints.forEach((nearbyPoint) => {
          // 检查是否可以连接
          if (this.areCompatible(point, nearbyPoint)) {
            const distance = point.position.distanceTo(nearbyPoint.position);
            
            if (distance <= this.maxConnectDistance) {
              // 检查是否已存在连接
              const existingConnection = connections.find(
                (conn) =>
                  (conn.source.componentId === point.instanceId &&
                    conn.source.pointId === point.pointId &&
                    conn.target.componentId === nearbyPoint.instanceId &&
                    conn.target.pointId === nearbyPoint.pointId) ||
                  (conn.source.componentId === nearbyPoint.instanceId &&
                    conn.source.pointId === nearbyPoint.pointId &&
                    conn.target.componentId === point.instanceId &&
                    conn.target.pointId === point.pointId)
              );
